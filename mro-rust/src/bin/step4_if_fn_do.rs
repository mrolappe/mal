...
fn eval_do<'a>(env: &'a mut Env, forms: &[MalData]) -> Result<MalData, EvalError> {
    forms.iter().map( |form| eval_ast(&form, env) ).last().unwrap()
}

fn eval_if<'a>(env: &'a mut Env, cond_form: &MalData, then_form: &MalData, else_form: Option<&MalData>) -> Result<MalData, EvalError> {
    match eval(env, cond_form) {
        Ok(MalData::Nil)  | Ok(MalData::False)  =>
            if else_form.is_some() {
                eval(env, else_form.unwrap())
            } else {
                Ok(MalData::Nil)
            },
...
fn eval<'a>(env: &'a mut Env, ast: &'a MalData) -> Result<MalData, EvalError> {
    // FIXME lifetime
    match ast {
        & MalData::List(ref list) => {
            ...
            if let Some(& MalData::Symbol(ref sym)) = list.first() {
                match sym.as_str() {
                    "def!" => {
                        return match ( list.get(1), list.get(2) ) {
                            ...
                            ( Some(& MalData::Symbol(ref key)), Some(value) ) => {
                                eval(env, value).map( |evaluated| { env.set(&key, &evaluated); evaluated })
                            }
                    ...
                    "let*" => {
                        let let_env = &mut Env::new(Some(env), vec!().iter(), vec!().iter()).unwrap();
                        ...
                        loop {
                            match ( iter.next(), iter.next() ) {
                                ( Some(&MalData::Symbol(ref sym)), Some(def) ) => {
                                    let evaluated_def = eval(let_env, def);
                        ...
                        return eval(let_env, let_body.unwrap());
                    }
        ...
        MalData::Map(ref map) => {
            ...
            loop {
                match iter.next() {
                    ...
                    Some(( k, v )) => {
                        eval_map.insert(k.clone(), eval(env, &v).unwrap());
                    }
                }
            }
...
fn rep(input: &str, repl_env: &mut Env) -> Result<String, EvalError> {
...

    {
        ...
        let eval_out = eval(repl_env, foo);
...
fn main() {
    ...
    let repl_env = &mut Env::new(None, binds.iter(), exprs.iter()).unwrap();
